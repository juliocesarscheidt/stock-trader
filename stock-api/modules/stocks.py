...
RETRY_SECONDS = int(os.environ.get('RETRY_SECONDS', '3'))
  ...
  def find_last_stock(self, args: dict):
    ...
    try:
      if self.mongoConnection.history_collection is None:
        self.connect_mongo_and_get_collection()
      # sort descending
      histories = self.mongoConnection.history_collection.find(args).sort('date', -1).skip(0).limit(1)
      log(histories)
      ...
      while True:
        try:
          if 'country' in args:
            ...
            publish(channel, args, RABBITMQ_EXCHANGE, RABBITMQ_ROUTING_KEY)
          return None
        except Exception as e:
          retries += 1
          log(e)
...
class LastStocks(Stock):
  def get(self):
    ...
    try:
      ...
      log(e)
      response = jsonify(status='Failure', data={'message': 'Internal Server Error'})
      return_code = 500
